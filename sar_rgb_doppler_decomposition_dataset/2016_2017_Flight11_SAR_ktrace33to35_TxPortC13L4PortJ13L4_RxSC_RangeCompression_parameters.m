...
out_fileSufix = '';
% file with 'Timestamp in UTC', 'Lat', 'Long', 'Ellipsoidal Height(WGS84)', 'Roll', 'Pitch', 'Heading'...
attribute_file = '.../F11.ASC';
...
endTraceDiffractionRegion = 1000000; %[1e3*100]; % number of trace
%-------------------------------------------------------------
% radar parameters
%-------------------------------------------------------------
% carrier frequency
...
integr_angle = 2.22044604925031e-16; % [deg]
%-------------------------------------------------------------
...
azGrid = (33*pulsesPerBlock+1):az_pres*1:(36*pulsesPerBlock); % [samples], (index starts at 1)
...
%-------------------------------------------------------------
firstRowIMUfile = '1'; % row in 'IMUfile'
lastRowIMUfile = '165850'; % row in 'IMUfile'
...
GPSManualOffsetInPulses = 125; % [pulses]
...
lastRowBotfile = 76499; % row in 'BotFile'
...
filterNoisyPeakReal_flag = 0; % 1:filter; 0:no filter
...
dopplerFilter_flag = 0; % 1:boxcar filter; 0:no filter
% Rotation correction
...
RxCalDependOnTx_flag = 0; % 0: as a column vector for the sets and a row vector for the receivers; 1: as matrices (receivers x sets)
% Constant calibration correction source
constantAPDCalibrationCorrectionFile_flag = 1; % 0: manually (within the actual file); 1: from calibration file
...
if netCDF_flag
    ...
    cellGlobalAttNetCDF{indexGA}.value = [... % summary of PASIN2
          ...
          ... % summary of current netCDF 
              'This netCDF file corresponds to a SAR image of the data collected during the Antarctic Summer of 2016/17, ' ...
              'above the Filchner Ice Shelf System, in the Recovery Ice Stream, near the grounding line. The SAR image is 2D, with ' ...
    ...
    cellGlobalAttNetCDF{indexGA}.value = 'Airborne ice-penetrating synthetic aperture radar, image by Backprojection'; % for identifying in DoA estimation
