% Display Rutford Ice Stream SAR images and SAR-derived products
%
%
%
%  
...
% Image thresholds
dBLim = [0 100]; % (dB), maximum lower limit below the maximum
% Image mosaicking type
mosaic_type = 3; % 0: no mosaicking; 1: 2x2; 2: 3x1; 3: 4x1
% Palette parameters for universal readability
if mosaic_type == 3
    palette_codes = [-1 -2 -3 3 0];
    blindness_type = 'none'; % 'none', 'protanopia', 'deuteranopia', 'tritanopia' and 'achromatopsia'
    ordinary_display_flag = true; % displays: B&W SAR; RGB; RGB_unique; overlap_RGB
else
    palette_codes = [-3 3];
    blindness_type = 'none'; % 'none', 'protanopia', 'deuteranopia', 'tritanopia' and 'achromatopsia'
    ordinary_display_flag = false; % displays: extra_modRGB; modRGB; modRGB_unique
    extraordinary_palette = palette_codes(1);
end
% Display parameters
longer_image = false;
aslFlag = false; % 0, thickness; 1, height above sea level
if ~aslFlag % thickness
    vertical_limits_shown = [-180 2500]/1e3; % [km]
else
    vertical_limits_shown = [-2200 400]/1e3; % [km]
end
along_track_limits = [7702  8162]/1e3; % [kilotraces]
% Change how to display the along-track axes
display_track_distance_flag = true; % false: time; true: distance
number_display_intervals = 5;
precission_m = 100; % [m]
...
if longer_image
    chosen_axes_relative_size = [0.75 0.33]; % [width_ratio, height_ratio]
else
    chosen_axes_relative_size = [1 1]; % [width_ratio, height_ratio]
end
...
RGB_over_full_universal_disp_flag = true;
RGB_over_unique_universal_disp_flag = true;
%----------------------------------------------------------------
%----------------------------------------------------------------
%----------------------------------------------------------------
%
...
for palette_code = palette_codes
    ...
    if RGB_full_universal_disp_flag
        ...
        auxCBarLabel = auxCBar.Label; set(auxCBarLabel,'String','Doppler spectrum (Hz)','Rotation',-90,'VerticalAlignment','bottom'); 
        set(auxCBar,'Ticks',(unique(RGBFreqLimitsNorm64)-1)/(numberOfColors-1),'TickLabels',strtrim(cellstr(num2str(unique(round(RGBFreqLimits.'/100*foundPRF/recPulseDiff*10)/10)))))
        if not(ordinary_display_flag) && extraordinary_palette==palette_code
            extraordinary_azFilteredRGBmod = azFilteredRGBmod;
            extraordinary_modRGBcolorbar = modRGBcolorbar;
        end
    end
    % Display image with the intensity of the most intense band in each SAR sample
    if RGB_unique_universal_disp_flag
        ...
        set(auxCBar,'Ticks',(unique(RGBFreqLimitsNorm64)-1)/(numberOfColors-1),'TickLabels',strtrim(cellstr(num2str(unique(round(RGBFreqLimits.'/100*foundPRF/recPulseDiff*10)/10)))))
        if not(ordinary_display_flag) && extraordinary_palette==palette_code
            extraordinary_azFilteredRGBmod_unique = azFilteredRGBmod_unique;
            extraordinary_modRGBcolorbar = modRGBcolorbar;
        end
    end
    % Display image with the the most intense band in each SAR sample
...
if mosaic_type
    ...
    switch mosaic_type
        ...
        case 2 % 3x1
            ...
            axis_position_cm = cell(numel(mosaic_axes),1);
            % Figure size
            mosaic_figure_position_norm = [2    2   17.4   14.4-3]; % 'mosaic_figure_units'
            % Labels
            xlabel_cell = {mosaic_xlabel_text, '', mosaic_xlabel_text};
            ylabel_cell = {mosaic_ylabel_text, mosaic_ylabel_text, mosaic_ylabel_text};
            % Definitions
            bottom_offset = 0.9; % [axes_units]
            left_offset = 1.05; % [axes_units]
            axis_height = 4.0-1.0; % [axes_units]
            axis_width = 14.65; % [axes_units]
            vertical_offset = 0.1 + axis_height; % [axes_units]
            colorbar_x_offset = 0.1; % [axes_units]
            colorbar_width = 0.3; % [axes_units]
            colorbar_height_ratio = 0.9; % [axes_units]
            colorbar_y_offset = (1-colorbar_height_ratio)/2*axis_height; % [axes_units]
            % Assignations
            axis_position_cm{1} = [left_offset, bottom_offset+2*vertical_offset, axis_width, axis_height];
            axis_position_cm{2} = [left_offset, bottom_offset+1*vertical_offset, axis_width, axis_height];
            axis_position_cm{3} = [left_offset, bottom_offset+0*vertical_offset, axis_width, axis_height];
            colorbar_position_cm{1} = [sum(axis_position_cm{1}([1 3]))+colorbar_x_offset, axis_position_cm{1}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
            colorbar_position_cm{2} = [sum(axis_position_cm{2}([1 3]))+colorbar_x_offset, axis_position_cm{2}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
            colorbar_position_cm{3} = [sum(axis_position_cm{3}([1 3]))+colorbar_x_offset, axis_position_cm{3}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
        case 3 % 4x1
            mosaic_axes = gobjects(4,1);
            axis_position_cm = cell(numel(mosaic_axes),1);
            % Figure size
            mosaic_figure_position_norm = [2    2   17.4   18.5-3]; % 'mosaic_figure_units'
            % Labels
            xlabel_cell = {mosaic_xlabel_text, '', '', mosaic_xlabel_text};
            ylabel_cell = {mosaic_ylabel_text, mosaic_ylabel_text, mosaic_ylabel_text, mosaic_ylabel_text};
            % Definitions
            bottom_offset = 0.9; % [axes_units]
            left_offset = 1.05; % [axes_units]
            axis_height = 4-1.0; % [axes_units]
            axis_width = 14.65; % [axes_units]
            vertical_offset = 0.1 + axis_height; % [axes_units]
            colorbar_x_offset = 0.1; % [axes_units]
            colorbar_width = 0.3; % [axes_units]
            ...
            colorbar_y_offset = (1-colorbar_height_ratio)/2*axis_height; % [axes_units]
            % Assignations
            axis_position_cm{1} = [left_offset, bottom_offset+3*vertical_offset, axis_width, axis_height];
            axis_position_cm{2} = [left_offset, bottom_offset+2*vertical_offset, axis_width, axis_height];
            axis_position_cm{3} = [left_offset, bottom_offset+1*vertical_offset, axis_width, axis_height];
            axis_position_cm{4} = [left_offset, bottom_offset+0*vertical_offset, axis_width, axis_height];
            colorbar_position_cm{1} = [sum(axis_position_cm{1}([1 3]))+colorbar_x_offset, axis_position_cm{1}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
            colorbar_position_cm{2} = [sum(axis_position_cm{2}([1 3]))+colorbar_x_offset, axis_position_cm{2}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
            colorbar_position_cm{3} = [sum(axis_position_cm{3}([1 3]))+colorbar_x_offset, axis_position_cm{3}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
            colorbar_position_cm{4} = [sum(axis_position_cm{4}([1 3]))+colorbar_x_offset, axis_position_cm{4}(2)+colorbar_y_offset, colorbar_width, axis_height*colorbar_height_ratio];
    end
    axis_font_size = 9;
    bar_font_size = 9;
    % Build mosaic
    ...
    if length(mosaic_axes) >= axis_index
        ...
        if ordinary_display_flag
            ...
            set(gca, 'XAxisLocation', 'top', 'GridColor', [1 1 1], 'GridLineStyle', '--')
            xlabel(xlabel_cell{axis_index}), ylabel(ylabel_cell{axis_index}),
            title(['SAR, ' recFlight ', ' TXWVstring(1:6) '-' RXstring ', beam center ' num2str(recAlongTrackCentralAngle) '\circ, aperture ' num2str(recAlongTrackApertureAngle) '\circ'])
            used_title = get(mosaic_axes(axis_index), 'Title');
            set(used_title,'Units','centimeters')
            set(used_title,'Position',get(used_title,'Position') - [0 0.2 0])
            colormap(gca, 'gray'),
            ...
            set(auxCBarLabel,'String','Amplitude (dB)','Rotation',-90,'VerticalAlignment','bottom');
            set(auxCBar,'Ticks',linspace(-max(dBLim),-min(dBLim),3),'TickLabels',round(linspace(-max(dBLim),-min(dBLim),3), 1)), clear auxCBar,
        else
            if ~aslFlag % thickness
                imagesc(alongTrackAxis, recRadialDepthGrid/1e3, extraordinary_azFilteredRGBmod), grid on,
            else % above sea level
                imagesc(alongTrackAxis, heightASLrecRadialDepthGrid/1e3, extraordinary_azFilteredRGBmod), grid on, axis xy,
            end
            set(gca, 'FontSize', axis_font_size, 'Color', [0.8 0.8 0.8], 'Units', mosaic_axes_units, 'Position', axis_position_cm{axis_index}),
            set(gca, 'XAxisLocation', 'top', 'GridColor', [1 1 1], 'GridLineStyle', '--')
            xlabel(xlabel_cell{axis_index}), ylabel(ylabel_cell{axis_index}),
            ...
            set(used_title,'Units','centimeters')
            set(used_title,'Position',get(used_title,'Position') - [0 0.2 0])
            colormap(gca, extraordinary_modRGBcolorbar),
    ...
    if length(mosaic_axes) >= axis_index
        ...
        set(gca, 'FontSize', axis_font_size, 'Color', [0.8 0.8 0.8], 'Units', mosaic_axes_units, 'Position', axis_position_cm{axis_index}),
        set(gca, 'GridColor', [1 1 1], 'GridLineStyle', '--'),
        xlabel(xlabel_cell{axis_index}), ylabel(ylabel_cell{axis_index}),
...
for axis_index = 1:length(single_axes)
    axis_size = get(single_axes(axis_index), 'Position');
    new_axis_size = [axis_size(1) axis_size(2) axis_size(3:4).*chosen_axes_relative_size];
    set(single_axes(axis_index), 'Position', new_axis_size);
end
...
if display_track_distance_flag
    ...
    if mosaic_type
        for axis_index = 1:length(mosaic_axes)
            if isempty(xlabel_cell{axis_index})
                ...
                used_label = get(mosaic_axes(axis_index), 'XLabel');
                set(used_label,'Units','centimeters')
                if axis_index == 1
                    % Lower position of top label
                    set(used_label,'Position',get(used_label,'Position') - [0 0.1 0])
                elseif axis_index == length(mosaic_axes)
                    % Higher position of bottom label
                    set(used_label,'Position',get(used_label,'Position') + [0 0.1 0])
                end
            end
        end
    ...
    % Special labels of the x-ticks
    xlabel(mosaic_axes(1), xLabelString)
    xticklabels(mosaic_axes(1), round(alongTrackAxis(index_closest_distance),2)); % [km]
    xticklabels(mosaic_axes(2:end-1), ''); % [km]
end
...
if add_box_labels_in_mosaic_flag
    ...
    vertical_shift = (length(mosaic_axes)-1): -1 : 0;
    for box_index = 1:length(vertical_shift)
        % char(97) = 'a'
        ...
        t.Position = [left_offset+textbox_h_offset, ...
                      bottom_offset + vertical_shift(box_index)*vertical_offset + axis_height - (textbox_v_offset + textbox_height), ...
                      textbox_width,...
                      textbox_height ];
    end
end